...
const colors = [
    { color: "wila", automata: new Automata("wila", alphabet) },
    {color: "janq'u", automata:new Automata("janq'u",alphabet)},
    { color: "q'illu",automata: new Automata("q'illu", alphabet) },
    { color: "ch'uxña", automata: new Automata("ch'uxña", alphabet) },
    {color: "anti", automata:new Automata("Anti",alphabet)},
    { color: "Arumi",automata: new Automata("Arumi", alphabet) },
    { color: "larama", automata: new Automata("larama", alphabet) },
...
function checkAnswer() {
    ...
    inputs.forEach((input, index) => {
        ...
        const colorData = colors[index]; // Obtener el objeto de color correspondiente

        if (!colorData) {
            console.log("Error: No hay datos de color correspondientes para el input en el índice", index);
            return; // Salir de la iteración si no hay datos de color correspondientes
        }
        ...
        for (let letter of userInput) {
            ...
            if (!automata.transition(letter)) {
                console.log("Transición fallida para el color:", colorData.color);
                break;
            }
        }
        ...
        if (automata.isAccepted()) {
            console.log("Transición exitosa para el color:", colorData.color);
            isCorrect = true;
        }
